...
function CustomersPage() {
  ...
  function create() {
    ...
    const action = dispatch(addCustomer({ name: name.trim(), phone: phone.trim(), email: email.trim(), address: address.trim(), notes: notes.trim() }));
  ...
  function saveEdit() {
    dispatch(updateCustomer({ id: editingId, ...edit, loyalty: Number(edit.loyalty) || 0, credit: Number(edit.credit) || 0 }));
                ...
                <td>
                  {editingId === c.id ? (
                    ...
                    <>
                      ...
                      <button className="btn" onClick={() => remove(c.id)} style={{ marginLeft: 6 }}>Remove</button>
                    </>
                  )}
                </td>
              </tr>
