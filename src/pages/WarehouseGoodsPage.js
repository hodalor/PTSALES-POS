        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 12 }}>
          {rows.map(product => (
            <div key={product.id} className="card" style={{ display: 'grid', gap: 8 }}>
              ...
              <div style={{ color: product.warehouseStock <= Number(product.lowStock || 0) ? '#b91c1c' : '#15803d', fontWeight: 700 }}>
            ...
            <tbody>
              {rows.map(product => (
                <tr key={product.id}>
                  ...
                  <td>{product.warehouseStock <= Number(product.lowStock || 0) ? 'Low stock' : 'Available'}</td>
                </tr>
              ))}
